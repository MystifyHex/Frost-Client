            ...
            Object.entries(mongooseData.channels).forEach(async (element) => {
                parser
                    ...
                    .then(async (ytData) => {
                        ...
                        if (data.postedVideos.includes(ytData.items[0].id))
                            return;
                    ...
                    .catch((error) => {
                        console.log(
                            `Error auto-posting youtube video: ${error}`
                        );
                    });
            });
        }, 5000);
...
client.player
    ...
    .on("playlistAdd", (message, _, playlist) =>
        message.channel.send(
            `\`${playlist.title}\` has been added to the queue (\`${playlist.tracks.length}\` songs), requested by \`${message.author.tag}\``
        )
    )
